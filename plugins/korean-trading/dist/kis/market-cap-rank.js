...
// scripts/kis/market-cap-rank.ts
...
async function main() {
  ...
  try {
    const json = await kisGet("/uapi/domestic-stock/v1/ranking/market-cap", "FHPST01740000", {
    ...
    let items = (json.output ?? []).filter((r) => r.mksc_shrn_iscd).map((r) => ({
