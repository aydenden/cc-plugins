...
- **인덱스가 계속 커진다** — CC는 `MEMORY.md`의 앞부분(25,000B / 200줄)만 세션 시작에 로드한다. write 직후 경고는 주지만 **write 자체는 통과**시키고, 넘은 부분은 다음 로드에서 잘린다 → 매 세션 거대 텍스트 주입(오염) + 뒷부분 누락(그 세션에 존재하지 않는 지식이 된다).
- **토픽 파일은 리콜에서 앞부분만 보인다** — CC 리콜은 파일마다 frontmatter `description`만 보고 고르고 앞 4,096B / 200줄만 보여준다(2.1.270 확인). 커진 토픽을 `-2`·`.1`로 이어 쪼개면 뒷조각은 거의 안 찾히고, description이 없으면 아예 고를 근거가 없다.
...
| | 판정 근거 | 어디에 |
|---|---|---|
| **불변** | 하드캡·리콜 한도(claude 실행파일에 박힌 상수), 인덱스만 매 세션 로드된다는 비용 구조, 리콜이 description으로 고른다는 사실, 깨진 링크·고아 토픽·노후 | 코드 |
...
토픽 예산 = 리콜 한도(4,096B / 200줄) × headroom  →  3,686B / 180줄
```
...
- 검출: 예산·하드캡 초과, 깨진 내부 링크(`](x.md)`, `[[wiki]]`), **고아 토픽**(인덱스가 어디서도 언급하지 않는 파일), 노후(본문 최신 날짜 90일 경과), **토픽 파일의 리콜 한도·예산 초과**, **frontmatter `description` 누락**.
- 파일·줄마다 반복되는 지적(긴 항목, description 누락)은 kind당 한 줄로 접는다.
- 후보가 있으면 `exit 2` → `asyncRewake`가 결과를 **메인 세션에 system reminder로 주입** → 에이전트가 의미 판단 후 처리. 후보가 없으면 조용히 종료.
- 커진 섹션·토픽은 **병합·정리가 먼저**, 그래도 크면 제 이름의 독립 사실 파일로 옮기라고 안내한다. 번호를 이어 붙인 분할은 권하지 않는다.
- **자동 삭제하지 않는다** — 지우기 전에 사용자에게 확인한다. `archive/`로 옮기는 것은 숨김 수단이 아니다: CC는 하위 폴더까지 메모리로 읽는다.
...
기본 제외: `MEMORY.md`, `README.md`, `_*`, `.*`, `*.draft.md`. 하위 디렉터리(`archive/` 등)는 스캔하지 않으므로 그리로 옮긴 파일은 이 점검에서 조용해진다. 다만 CC 자신은 하위 폴더도 읽으므로 리콜에서까지 사라지지는 않는다.
